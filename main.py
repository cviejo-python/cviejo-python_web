...
def login(): # Con esta funcion autenticamos a un usuario y abreimos sesion.
    ...
    if request.method == "POST":
        ...
        # Obtenemos el email y contraseña escritos en el formulario
        ...
        # 2. Que la contraseña introducida coincida con el hash guardado
    ...
    # Si el método es GET mostramos la plantilla login.html
...
def registro():
    ...
    if request.method == "POST":
        ...
        # Hacemos login automático guardando el id en sesión
...
def pagar():
    ...
    # nos aseguramos que el usuario elija una dirección
    if not direccion_id:
        return redirect(url_for("direcciones"))
    ...
    for id_producto, datos in carrito.items():
        ...
        if producto.stock < datos["cantidad"]:
            flash(
                f"{producto.nombre_producto} no tiene suficiente stock disponible "
                f"en este momento. "
                f"Estamos reponiendo existencias lo antes posible.",
                "warning"
            )
...
def cargar_datos_prueba():
    ...
    usuarios = [
        Usuario("Juan","Pérez","juan1@email.com",generate_password_hash("1234"),"600000001",False),
